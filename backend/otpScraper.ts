...
const TARGET_TIER = getArg('tier') ? [getArg('tier')!.toUpperCase()] : ["CHALLENGER", "GRANDMASTER", "MASTER"];

const WEBSITE_DB_PATH = path.join(__dirname, 'otp_data_v4.json');
const AI_DATASET_PATH = path.join(__dirname, 'ai_dataset_raw.jsonl');

console.log(`🔧 CONFIG: Region=${REGION_ID}, Pages=${MAX_PAGES}, Deep=${DEEP_SCAN}, Tiers=${TARGET_TIER.join(',')}`);
...
setInterval(() => { if (requestTokens < 18) requestTokens++; }, 1200); // Regeneracja
...
const fetchTimeline = async (matchId: string): Promise<TimelineData | null> => {
    ...
    return fetchJson(`https://${REGION_ROUTING}.api.riotgames.com/lol/match/v5/matches/${matchId}/timeline`);
};
...
export const runOtpScraper = async () => {
    ...
    if (fs.existsSync(WEBSITE_DB_PATH)) {
        try { websiteDatabase = JSON.parse(fs.readFileSync(WEBSITE_DB_PATH, 'utf-8')); } catch { /* ignore */ }
    }
                            ...
                            if (p.championId === mainId && p.win && match.info.gameDuration > 1000 && kda > 2.0) {
                                ...
                                if (DEEP_SCAN) {
                                    ...
                                    if (timeline) {
                                        ...
                                        const pId = timeline.info.participants.find(tp => tp.puuid === puuid)?.participantId;
                                        if (pId) {
                                            // A. Skill Order & Build
                                            timeline.info.frames.forEach(frame => {
                                                frame.events.forEach(ev => {
                                                    if (ev.participantId === pId) {
                                                        if (ev.type === 'SKILL_LEVEL_UP' && ev.skillSlot && ev.levelUpType === 'NORMAL') {
