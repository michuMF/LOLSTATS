...
export const initializeDataDragon = async () => {
    try {
        ...
        const itemsRes = await fetch(`https://ddragon.leagueoflegends.com/cdn/${v}/data/pl_PL/item.json`);
        const itemsData = await itemsRes.json() as DDItemData;
        Object.keys(itemsData.data).forEach((id) => { ITEM_MAP[id] = itemsData.data[id].name; });

        console.log("⏳ [3/4] Data Dragon: Nauka run...");
        const runesRes = await fetch(`https://ddragon.leagueoflegends.com/cdn/${v}/data/pl_PL/runesReforged.json`);
        const runesData = await runesRes.json() as DDRuneTree[];
        runesData.forEach((tree) => {
            RUNE_MAP[tree.id] = tree.name;
            tree.slots.forEach((slot) => {
                slot.runes.forEach((rune) => { RUNE_MAP[rune.id] = rune.name; });
            });
        });
        ...
        Object.values(champData.data).forEach((champ) => { CHAMPION_TAGS[champ.name] = champ.tags; });
