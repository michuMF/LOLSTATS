  ...
  const summary = {
    ...
    csDiff: enemy
      ? me.totalMinionsKilled +
        ...
        (enemy.totalMinionsKilled + (enemy.neutralMinionsKilled || 0))
      : 0,
    damageShare:
      (
    ...
    enemyLaner: enemy
      ? {
          ...
          items: [enemy.item0, enemy.item1, enemy.item2, enemy.item3, enemy.item4, enemy.item5]
            .filter((id) => id !== 0)
            .map((id) => ITEM_MAP[id] || `${id}`),
        }
      : null,
...
export async function analyzeMatchWithAI(s: AiMatchSummary) {
  ...
  const prompt = `
    ...
    - Zbudował przedmioty (ID): ${s.expertBenchmark.buildPath
      .map((b) => b.itemId)
      .slice(0, 3)
      .join(", ")}.
