...
const loadDb = () => {
    ...
    try {
        if (fs.existsSync(DB_PATH)) {
            ...
            console.log(`📘 [ExpertData] Załadowano dane eksperckie: ${Object.keys(expertDb || {}).length} wpisów.`);
        } else {
...
export const getExpertGuide = (championId: number, enemyChampionId: number = -1): ExpertGuide | null => {
    const db = loadDb();
    if (!db) return null;
