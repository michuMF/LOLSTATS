...
export const MetaDetailsPanel = ({
  champion,
  tier,
  metaData,
  version,
  onClose,
}: MetaDetailsPanelProps) => {
        ...
        Object.values(metaData).forEach((tierData) => {
          ...
          if (stats) {
            ...
            if (stats.marketing && aggregated.marketing) {
              const sumBucket = (
                target: Record<string, { picks: number; wins: number }>,
                source: Record<string, { picks: number; wins: number }>
              ) => {
              ...
              if (stats.marketing.keystones)
                sumBucket(aggregated.marketing.keystones, stats.marketing.keystones);
              if (stats.marketing.secondaryTrees)
                sumBucket(aggregated.marketing.secondaryTrees, stats.marketing.secondaryTrees);
              if (stats.marketing.spells)
                sumBucket(aggregated.marketing.spells, stats.marketing.spells);
            }
          }
        });
      ...
      <div className="bg-slate-800/95 backdrop-blur-xl border border-slate-700 rounded-2xl p-6 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-6 mb-8 pb-6 border-b border-white/5 relative z-10">
          ...
          <div className="flex-1">
            ...
            <p className="text-yellow-500 font-mono text-sm uppercase tracking-wider opacity-80">
              {champion.title}
            </p>
          ...
          {analysis && (
            <div className="text-right px-2 hidden sm:block">
              ...
              <div
                className={`text-5xl font-black ${getWrColor(analysis.overallWinRate)} drop-shadow-lg`}
              >
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center border border-slate-600">
                        <span className="text-xs font-bold text-slate-400">
                          {analysis.runes.secondaryTree?.id}
                        </span>
                      </div>
              ...
              {analysis.spells.length > 0 && (
                <div className="flex-1 bg-slate-900/40 p-4 rounded-xl border border-slate-700/50">
                  ...
                  <div className="mt-2 text-xs text-slate-400">
                    Win Rate:{" "}
                    <span className={getWrColor(analysis.spells[0].winRate)}>
                      {analysis.spells[0].winRate}%
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
              ...
              <div>
                <h3 className="flex items-center gap-2 text-lg font-bold text-white mb-4 border-b border-slate-700/50 pb-2">
                  ...
                  Most Popular{" "}
                  <span className="text-slate-500 text-xs font-normal ml-auto">(Pick Rate)</span>
                </h3>
                ...
                <div className="space-y-3">
                  {analysis.coreItems.map((item, i) => (
                    <div
                      key={item.id}
                      className="flex items-center bg-slate-900/40 p-2.5 rounded-xl border border-slate-700/50 hover:bg-slate-800/60 hover:border-orange-500/30 transition-all group"
                    >
                      <span className="w-6 text-center text-slate-600 font-mono text-xs font-bold">
                        #{i + 1}
                      </span>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-baseline mb-1.5">
                          <div className="text-xs font-bold text-slate-200">
                            {item.pickRate}%{" "}
                            <span className="text-slate-500 font-normal">Pick</span>
                          </div>
              ...
              <div>
                <h3 className="flex items-center gap-2 text-lg font-bold text-white mb-4 border-b border-slate-700/50 pb-2">
                  ...
                  Highest Winrate{" "}
                  <span className="text-slate-500 text-xs font-normal ml-auto">(Situational)</span>
                </h3>
                ...
                <div className="grid grid-cols-4 sm:grid-cols-4 xl:grid-cols-3 gap-3">
                  {analysis.situationalItems.map((item) => (
                    <div
                      key={item.id}
                      className="relative group bg-slate-900/40 p-2 rounded-xl border border-slate-700/50 hover:border-blue-500/50 hover:bg-slate-800/80 transition-all flex flex-col items-center justify-center text-center"
                    >
                      {/* Obrazek */}
                ...
                <div className="mt-4 p-3 bg-blue-500/10 rounded-lg border border-blue-500/20">
                  <p className="text-[10px] text-blue-300 leading-relaxed text-center">
                    Te przedmioty są rzadziej wybierane, ale mają bardzo wysoką skuteczność. Rozważ
                    je jako opcje sytuacyjne.
                  </p>
                </div>
              </div>
            </div>
