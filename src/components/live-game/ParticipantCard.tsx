...
export const ParticipantCard = ({ 
    participant, isBlueTeam, region, championName, 
    getSpellId, getRuneIcon, version 
  ...
  // --- 3. RESZTA UI ---
  const teamColorClass = isBlueTeam ? "border-l-4 border-blue-500 bg-blue-50/50" : "border-r-4 border-red-500 bg-red-50/50";
  ...
  const keystoneIcon = participant.perks?.perkIds?.[0] ? getRuneIcon(participant.perks.perkIds[0]) : null;
  const subStyleIcon = participant.perks?.perkSubStyle ? getRuneIcon(participant.perks.perkSubStyle) : null;

  return (
    <div className={`relative flex items-center justify-between p-3 mb-2 rounded shadow-sm bg-white ${teamColorClass} transition hover:scale-[1.01]`}>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col gap-1">
           <div className="flex gap-1">
              {spell1 && <img src={`https://ddragon.leagueoflegends.com/cdn/${version}/img/spell/${spell1}.png`} className="w-5 h-5 rounded" alt="Spell1" />}
              {spell2 && <img src={`https://ddragon.leagueoflegends.com/cdn/${version}/img/spell/${spell2}.png`} className="w-5 h-5 rounded" alt="Spell2" />}
           </div>
           <div className="flex gap-1 justify-center items-center">
              {keystoneIcon && <img src={`https://ddragon.leagueoflegends.com/cdn/img/${keystoneIcon}`} className="w-5 h-5 bg-slate-800 rounded-full p-0.5" alt="Key" />}
              {subStyleIcon && <img src={`https://ddragon.leagueoflegends.com/cdn/img/${subStyleIcon}`} className="w-5 h-5 bg-slate-800 rounded-full p-1 opacity-90" alt="Sub" />}
           </div>
        </div>
      </div>
      ...
      <div className={`flex-1 px-4 ${isBlueTeam ? "text-left" : "text-right"}`}>
         {isBot ? (
            ...
            <Link to={`/profile/${region}/${displayName.replace("#", "/")}`} className="font-bold text-slate-700 hover:text-blue-600 block truncate">
                {displayName}
            </Link>
         )}
