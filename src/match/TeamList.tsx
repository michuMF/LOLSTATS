  ...
  return (
    <div className={`flex items-center p-1.5 rounded transition-all ${rowClass}`}>
      ...
      <div className="flex-grow min-w-0 pr-2 flex flex-col justify-center">
        <div className="flex items-center gap-1">
          <span
            className={`text-xs truncate font-medium ${isMe ? "text-slate-900 font-bold" : "text-slate-600"}`}
          >
            {participant.riotIdGameName}
          </span>
        </div>
...
const TeamListComponent = ({
  ...
  <div className="bg-slate-50 p-4">
    <h4
      className={`text-xs font-bold uppercase tracking-widest mb-3 ${color} opacity-80 border-b border-slate-200 pb-1`}
    >
      {teamName}
    </h4>
