...
export const analyzeMatch = (
  ...
  const laneOpponent = enemyTeam.find((p) => p.teamPosition === player.teamPosition) || enemyTeam[0];

  const goldDiff = player.goldEarned - laneOpponent.goldEarned;
  const csDiff = (player.totalMinionsKilled + player.neutralMinionsKilled) - 
  ...
  const isFullAD = physRatio > 0.80;
  const isFullAP = magicRatio > 0.80;
  ...
  if (player.role !== "SUPPORT") {
      ...
      if (goldDiff > 1000) incomeScore += 20; // Zmiażdżył rywala
  ...
  if (winLane) {
      ...
      feedback.push(`Przegrałeś linię z ${laneOpponent.championName} (-${Math.abs(goldDiff)} złota).`);
  }
  ...
  if (totalScore >= 95) { grade = "SSS"; gradeColor = "text-yellow-500"; }
  else if (totalScore >= 85) { grade = "S"; gradeColor = "text-blue-500"; }
  else if (totalScore >= 75) { grade = "A"; gradeColor = "text-emerald-500"; }
  else if (totalScore >= 60) { grade = "B"; gradeColor = "text-teal-500"; }
  else if (totalScore >= 45) { grade = "C"; gradeColor = "text-orange-500"; }
