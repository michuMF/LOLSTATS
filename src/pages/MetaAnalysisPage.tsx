...
export const MetaAnalysisPage = () => {
  ...
  if (loading) return <div className="pt-20 flex justify-center"><LoadingSpinner /></div>;
  if (error) return <div className="pt-20 flex justify-center"><ErrorMessage message={error} /></div>;
  ...
  return (
    <div className="container mx-auto p-6 pt-10 text-white min-h-screen relative animate-fadeIn">
      ...
      <div className="text-center mb-10 mt-6">
        ...
        <p className="text-slate-400">Patch: <span className="text-yellow-400 font-mono">{version}</span> • Global Itemization Data</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start relative">
        ...
        {/* LISTA POSTACI */}
        <div className={`grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 xl:grid-cols-8 gap-3 content-start transition-all duration-500 ease-in-out ${selectedChamp ? 'lg:w-1/2 h-[calc(100vh-250px)] overflow-y-auto pr-2 custom-scrollbar' : 'w-full'}`}>
           {filteredChampions.map(champ => {
               const hasData = metaData && (selectedTier === 'ALL' 
